...
class ThrottlingMiddleware(BaseMiddleware):
    def __init__(self, client: Redis, limit=.5, key_prefix='anti_flood_'):
        self.rate_limit = limit
        self.prefix = key_prefix
        self.throttle_manager = ThrottleManager(client=client)
    ...
    async def __call__(
        ...
        print('event', event)
        try:
            await self.on_process_event(handler, event, data)
...
class ThrottleManager:
    ...
    async def throttle(self, key: str, rate: float, user_id: int, chat_id: int):
        ...
        data = await self.client.hmget(bucket_name, self.bucket_keys)
        data = {
            k: float(v.decode())
        ...
        await self.client.hmset(bucket_name, data)
